...
function setFilter_Entity() {
    ...
    var prodTipoPortfolio;

    if (tipoSolicitacao === portifolio) prodTipoPortfolio = 482870000;
    else if (tipoSolicitacao === portifolioPiloto) prodTipoPortfolio = 482870001;
    else if (tipoSolicitacao === novoProduto) prodTipoPortfolio = 482870002;
    ...
    if (tipoSolicitacao === portifolio) {
        ...
        if (entidade != null) {
            ...
            productIdControl.removePreSearch(function () { addFilterOnProductByCategory() });
            setTimeout(function () {
                productIdControl.addPreSearch(function () { addFilterOnProductByEntityPortfolio() });
            }, 100);
    ...
    if (tipoSolicitacao === portifolioCustomizado) {
        var entidade = Xrm.Page.data.entity.attributes.get("findes_coligadaid").getValue();
        if (entidade != null) {
            var productIdControl = Xrm.Page.getControl("productid");
            productIdControl.removePreSearch(function () { addFilterOnProductByCategory() });
            productIdControl.removePreSearch(function () { addFilterOnProductByEntity() });
            setTimeout(function () {
                productIdControl.addPreSearch(function () { addFilterOnProductByEntityCustom() });
            }, 100);
        } else {
            var productIdControl = Xrm.Page.getControl("productid");
            productIdControl.removePreSearch(function () { addFilterOnProductByCategory() });
            productIdControl.removePreSearch(function () { addFilterOnProductByEntity() });
            productIdControl.removePreSearch(function () { addFilterOnProductByEntityCustom() });
            Xrm.Page.data.entity.attributes.get("productid").setValue(null);
            defaultFrm();
        }
    ...
    if (tipoSolicitacao === portifolioPiloto) {
        ...
        if (entidade != null) {
            ...
            productIdControl.removePreSearch(function () { addFilterOnProductByEntity() });
            setTimeout(function () {
                productIdControl.addPreSearch(function () { addFilterOnProductByEntityPortfolioPiloto() });
            }, 100);
        } else {
            var productIdControl = Xrm.Page.getControl("productid");
            productIdControl.removePreSearch(function () { addFilterOnProductByCategory() });
...
function addFilterOnProductByEntityCustom() {
    ...
    var fetch = '<filter type="and">' +
        ...
        //'<condition attribute="findes_tipoportfolio" operator="eq" value="482870001" />' + //Portifolio Customizado
        '</filter>';
...
function addFilterOnProductByEntityPortfolio() {
    var productIdControl = Xrm.Page.getControl("productid");
    var aux = Xrm.Page.getAttribute("findes_coligadaid").getValue()[0];
    var id = aux.id;
    /*var fetch = '<filter type="and">' +
...
function Oculta_produtooportunidade() {
    ...
    var solicitacaoNovoProdutoControl = Xrm.Page.getControl("findes_solicitacao");

    //Campos Customização
    var produtoCustomizacaoControl = Xrm.Page.getControl("productid");
    var solicitacaoCustomizacaoControl = Xrm.Page.getControl("findes_solicitacao");

    //Xrm.Page.data.entity.attributes.get("CRM_LookUp").getValue();
    ...
    if (tipoSolicitacao === portifolio) {
        ...
        solicitacaoNovoProdutoControl.setVisible(false);

        //Esconde Customização
        solicitacaoCustomizacaoControl.setVisible(false);
    ...
    if (tipoSolicitacao === novoProduto) {
        ...
        solicitacaoNovoProdutoControl.setVisible(true);
    ...
    if (tipoSolicitacao === customizacao) {
        ...
        solicitacaoCustomizacaoControl.setVisible(true);
