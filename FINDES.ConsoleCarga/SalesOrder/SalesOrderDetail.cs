...
namespace FINDES.ConsoleCarga
{
    public class SalesOrderDetail
    {
        ...
        protected static string file = $"SalesOrderDetail_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > SalesOrderDetail > Started");
                ConsoleHelper.Log($"Method: {method}.");

                var fields = Fields.SalesOrderDetail.fields;
                int pageNumber = 1;

                #region | QueryExpression
                var query = new QueryExpression(Fields.SalesOrderDetail.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, Fields.SalesOrderDetail.tableSQL, Fields.SalesOrderDetail.entityName);
                #endregion

                ConsoleHelper.Log("Processing > SalesOrderDetail > Finished");
            }
            catch (Exception ex)
            {
