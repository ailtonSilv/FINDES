...
namespace FINDES.ConsoleCarga
{
    public class SalesOrder
    {
        ...
        protected static string file = $"SalesOrder_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > SalesOrder > Started");
                ConsoleHelper.Log($"Method: {method}.");

                var fields = Fields.SalesOrder.fields;
                int pageNumber = 1;

                #region | QueryExpression
                var query = new QueryExpression(Fields.SalesOrder.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, Fields.SalesOrder.tableSQL, Fields.SalesOrder.entityName);
                #endregion

                ConsoleHelper.Log("Processing > SalesOrder > Finished");
            }
            catch (Exception ex)
            {
