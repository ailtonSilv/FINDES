...
namespace FINDES.ConsoleCarga
{
    public class Estado
    {
        ...
        protected static string file = $"Estado_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > Estado > Started");
                ConsoleHelper.Log($"Method: {method}.");

                var fields = CargaFields.Estado.fields;
                int pageNumber = 1;

                #region | QueryExpression
                var query = new QueryExpression(CargaFields.Estado.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, CargaFields.Estado.tableSQL, CargaFields.Estado.entityName);
                #endregion

                ConsoleHelper.Log("Processing > Estado > Finished");
            }
            catch (Exception ex)
            {
