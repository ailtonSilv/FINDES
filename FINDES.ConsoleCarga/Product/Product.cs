...
namespace FINDES.ConsoleCarga
{
    public class Product
    {
        ...
        protected static string file = $"Product_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > Product > Started");
                ...
                var query = new QueryExpression(Fields.Product.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, Fields.Product.tableSQL, Fields.Product.entityName);
                #endregion

                ConsoleHelper.Log("Processing > Product > Finished");
            }
            catch (Exception ex)
            {
