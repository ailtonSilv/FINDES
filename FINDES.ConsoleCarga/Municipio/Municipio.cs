...
namespace FINDES.ConsoleCarga
{
    public class Municipio
    {
        ...
        protected static string file = $"Municipio_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > Municipio > Started");
                ConsoleHelper.Log($"Method: {method}.");

                var fields = CargaFields.Municipio.fields;
                int pageNumber = 1;

                #region | QueryExpression
                var query = new QueryExpression(CargaFields.Municipio.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, CargaFields.Municipio.tableSQL, CargaFields.Municipio.entityName);
                #endregion

                ConsoleHelper.Log("Processing > Municipio > Finished");
            }
            catch (Exception ex)
            {
