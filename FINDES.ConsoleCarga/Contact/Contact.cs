...
namespace FINDES.ConsoleCarga
{
    public class Contact
    {
        ...
        protected static string file = $"Contact_{DateTime.Now.ToShortDateString().Replace("/", "")}-{DateTime.Now.ToShortTimeString().Replace(":", "")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > Contact > Started");
                ...
                var query = new QueryExpression(Fields.Contact.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, Fields.Contact.tableSQL, Fields.Contact.entityName);
                #endregion

                ConsoleHelper.Log("Processing > Contact > Finished");
            }
            catch (Exception ex)
            {
