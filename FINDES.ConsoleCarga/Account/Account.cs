...
namespace FINDES.ConsoleCarga
{
    public class Account
    {
        ...
        protected static string file = $"Account_{DateTime.Now.ToShortDateString().Replace("/","")}-{DateTime.Now.ToShortTimeString().Replace(":","")}.csv";
        ...
        public static void Send(string method, bool onlyCargaInicial, bool testMode)
        {
            ...
            {
                ConsoleHelper.Log("");
                ConsoleHelper.Log("Processing > Account > Started");
                ...
                var query = new QueryExpression(Fields.Account.entityName);
                ...
                #region | PROCESS
                ConsoleHelper.ProcessQuery(method, query, fields, file, Fields.Account.tableSQL, Fields.Account.entityName);
                #endregion

                ConsoleHelper.Log("Processing > Accounts > Finished");
            }
            catch (Exception ex)
            {
